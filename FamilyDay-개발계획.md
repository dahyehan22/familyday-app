...
- [x] 비밀번호 재설정 기능 (`resetPasswordForEmail` + `PASSWORD_RECOVERY` 이벤트 + 새 비밀번호 입력 UI)
- [x] 회원가입 시 초대 코드 입력 → 가입 완료 후 자동 가족 합류
- [ ] 신규 회원가입 → 로그인 플로우 검증
- [ ] 초대 코드로 가족 합류 → 데이터 공유 검증
...
- [x] 기존 useState 데이터를 Supabase CRUD로 교체
  ...
  - [x] profiles: 조회/수정 (Supabase profiles 테이블 동기화)
  - [x] family_members: 조회/추가/수정/삭제
  - [x] coupons: 조회/추가/삭제
  - [x] coupon_usage: 사용 기록/조회
...
#### 구현 항목
- [x] 가족 그룹 생성 (첫 가입 시 자동)
- [x] 초대 코드 생성 (6자리, 24시간 유효, 1회 사용)
- [x] 초대 코드 입력 UI + 가족 합류 처리 (마이페이지에서 역할/아이콘 선택 후 합류)
- [x] 회원가입 시 초대 코드 입력 → 가입 완료 후 자동 가족 합류
- [x] 프로필 구성원 추가 (계정 없는 자녀 등)
- [ ] 가족 구성원 간 데이터 공유 검증
- [x] 구성원 역할 권한 (부모/조부모: 관리 기능, 아이: 할일 추가/쿠폰 사용만)
- [x] 본인 역할 수정 (프로필 카드에서 변경)
- [x] 구성원 수정 (이름/역할/아이콘 편집 모달)
- [x] 구성원 삭제 확인 다이얼로그
...
- [x] 쿠폰 사용 이력 조회 (부모용, 쿠폰 관리 페이지 하단)
- [x] 쿠폰 수정 기능 (쿠폰 관리 페이지에서 편집 아이콘 클릭)
- [ ] 별 레벨 시스템 (총 누적 별 기반 레벨 뱃지)
- [x] 연속 달성 배너 (2일 이상 연속 달성 시 홈에 배너 표시)
...
### 3-1. 레이아웃 & 네비게이션 개선
- [x] 상단 영역 고정 (position: sticky)
- [x] 스크롤 다운 시 하단 GNB 숨김 (스크롤 업 시 다시 표시, 300ms 트랜지션)

### 3-2. 일정 추가 UX 개선
- [x] 캘린더 탭에 플로팅 "일정 추가" 버튼 추가 (할일 추가 버튼과 동일 스타일)

### 3-3. 타임라인 개선
- [x] 타임라인 색상 3가지로 제한 (secondary, primary, accent)
- [x] 현재 시간 기준으로 타임라인 자동 감지 (하드코딩 → 실시간 시간 계산)
- [x] 타임라인 진입 시 현재 항목으로 자동 스크롤
